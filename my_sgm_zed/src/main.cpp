...
void GetMaskRoi(cv::Mat& img_left, cv::Size& siz_scale, cv::Mat& mask, cv::Mat& mask_scale, bool& has_roi, std::vector<int>& rect_roi, bool& run){
...
//获取目标区域函数参数设定
    ...
    std::thread GetRoi(GetMaskRoi, std::ref(img_zed_left_remap), std::ref(siz_scale), std::ref(mask), std::ref(mask_scale), std::ref(has_roi), std::ref(rect_roi), std::ref(run));
